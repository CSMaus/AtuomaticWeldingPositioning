...
class YoloFrameMeasurer:
    ...
    def measure_on_frame(self, frame, conf_thresh=0.3):
        ...
        # ---- cx (debounce + clamp + EWMA + median) ----
        cx_raw = (x1 + x2) // 2
        cx_c = self._clamp_jump(self.prev_cx if self.prev_cx is not None else cx_raw, cx_raw, cap_cx_px)
        cx_c = self.cx_db.step(self.prev_cx, cx_c)
        cx_s = self._smooth(self.prev_cx if self.prev_cx is not None else cx_c, cx_c, SMOOTH_BETA_CX)
        self.cx_buf.append(cx_s)
        cx = float(np.median(self.cx_buf))

        # ---- xL (debounce + clamp + EWMA) ----
        xL_c = self._clamp_jump(self.prev_xL if self.prev_xL is not None else xL_raw, xL_raw, cap_px)
        xL_c = self.xL_db.step(self.prev_xL, xL_c)
        xL = self._smooth(self.prev_xL if self.prev_xL is not None else xL_c, xL_c, SMOOTH_BETA_X)
